---
title: "Doing Meta-Analysis in R: A Hands-on Guide"
date: 2026-04-17
permalink: /posts/2026/04/doing-meta-analysis-in-r-series/
tags:
  - meta-analysis
  - R
  - reading notes
  - book notes
---

I am starting a reading-note series on *Doing Meta-Analysis in R: A Hands-on Guide*.

This series will summarize key methods, practical R workflows, and my own reflections on how the book can inform applied meta-analysis projects.

## Why this series

I want to use this space to:
- organize my reading notes systematically,
- connect the book's content to real meta-analytic practice,
- build a reusable reference for future projects.

## Planned notes

### 基本内容

- [Welcome!](/reading-notes/dmar-note-01/)
- [入门](/reading-notes/dmar-note-02/)
- [1 引言](/reading-notes/dmar-note-03/)
- [2 探索 R]({{ '/assets/pdf/Discovering-R.pdf' | relative_url }})
- [3 效应量](/reading-notes/dmar-note-05/)
- [4 合并效应量]({{ '/assets/pdf/4-Pooling-Effect-Sizes.pdf' | relative_url }})
- [5 研究间异质性]({{ '/assets/pdf/4-Between-Study-Heterogeneity.pdf' | relative_url }})
- [6 森林图]({{ '/assets/pdf/6-Forest-Plots.pdf' | relative_url }})
- [7 亚组分析]({{ '/assets/pdf/7-Subgroupanalysis.pdf' | relative_url }})
- [8 Meta 回归](/reading-notes/dmar-note-08/)
- [9 发表偏倚](/reading-notes/dmar-note-09/)

### 高级方法

- [10 “多层级”Meta 分析](/reading-notes/dmar-note-10/)
- 11 结构方程模型 Meta 分析
- [12 网络 Meta 分析](/reading-notes/dmar-note-12/)
- [13 贝叶斯 Meta 分析](/notes/bayesian-meta-analysis/)

### 实用工具

- 14 功效分析
- [15 偏倚风险图]({{ '/assets/pdf/15-Risk-of-Bias-Plots.pdf' | relative_url }})
- 16 报告与可重复性
- 17 效应量计算与转换
## How I will use these notes

These notes will focus on:
- conceptual understanding,
- practical R implementation,
- issues that matter in real research workflows.

I will continue updating this series as I read further.
