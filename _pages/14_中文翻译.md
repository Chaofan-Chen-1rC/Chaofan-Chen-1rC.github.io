...
在继续之前，我需要说明，本节在很大程度上受到了 [Matti Vourre](https://vuorre.netlify.app/) 那篇优秀博文 [*Bayesian meta-analysis with R, Stan & brms*](https://mvuorre.github.io/posts/2016-09-29-bayesian-meta-analysis/) 的影响。由于 McElreath 的教材并未直接讨论元分析，我们还需要从 Gelman、Carlin、Stern、Dunson、Vehtari 和 Rubin 的 [*Bayesian data analysis, Third edition*](https://stat.columbia.edu/~gelman/book/) 中借鉴一些内容。下面就让 Gelman 及其合作者为我们引入这一主题：
...
由于正文中没有提供元分析的例子，我们只好从别处获取数据。这里我们聚焦于 Gershoff 和 Grogan-Kaylor 的论文 [*Spanking and child outcomes: Old controversies and new meta-analyses*](https://doi.org/10.1037/fam0000191)。在其引言中，我们读到：
...
McElreath 在其教材中并未真正聚焦于效应量。如果你需要复习相关内容，可以参考 Kelley 和 Preacher 的 [*On effect size*](https://www.researchgate.net/profile/Ken_Kelley/publication/270757972_On_Effect_Size/links/0046351b0cd48217ce000000/On-Effect-Size.pdf)。简言之，Cohen's $d$ 是两个组之间标准化后的均值差。
...
当然，最后两行给出的就是我们的先验分布。在心理学中，Cohen's $d$ 的绝对值大于 $\pm 1$ 的情况相当少见。因此，在缺乏更具体领域知识的情况下，而我确实也没有这样的知识，$\operatorname{Normal}(0, 1)$ 看起来是一个合理的起点。并且，就像 McElreath 将 $\operatorname{HalfCauchy}(0, 1)$ 用作组层标准差的默认先验一样，[在这里将其用于](https://psyarxiv.com/7tbrm/) 我们元分析中的 $\tau$ 参数也是合理的。
...
这也引出了另一个问题。我们通常是在数据汇总层面上进行元分析。在我的领域中，也许在你的领域中也是如此，这在一定程度上源于一个历史性的偶然：研究者过去并不常规地公开其原始数据。因此，效应量汇总往往就是我们通常所能得到的最好材料。然而，时代正在改变。如果你的元分析所纳入研究的全部原始数据都可获得，那么你完全可以直接拟合一个多层模型，使数据嵌套在研究之中。甚至，你还可以通过采用[分布式建模方法](https://cran.r-project.org/package=brms/vignettes/brms_distreg.html#a-simple-distributional-model)让研究之间在 $\sigma$ 上也存在差异，并指定诸如 `sigma ~ 0 + study`，甚至 `sigma ~ 1 + (1 | study)` 这样的模型。
